...
* **Envelopes are functional.** The `signdocs_envelope` post type has been registered and visible in the admin since 1.2.0, and the listing has described multi-signer envelopes since then, but nothing was wired to it: there was no signer repeater, no document picker, and no code path that sent an envelope to the API. Creating one saved a title and did nothing. It now has all three.
  ...
  * **Combined PDF download** on a completed envelope — one document carrying every signature. The link is presigned and expires after a few hours, so the screen shows how long is left and offers to mint a new one rather than presenting a dead link that answers 403.
  * **Cancel an envelope** from its own screen. This goes through the envelope's cancel endpoint rather than cancelling each signer in turn: doing it one by one leaves the envelope itself active, costs a call per signer, and records several events instead of one auditable cancellation. Signatures already collected are preserved and reported back.
* **Registering the webhook now recovers from an existing registration.** The API refuses to register a URL it already holds, which is what stops a re-clicked button doubling a tenant's inbound event volume. The plugin offered no way past it: the only copy of the signing secret lives in this site's options, so once it was lost — a migration, a restored backup, a re-install — the admin could neither re-register nor recover it, and every delivery failed signature verification with nothing in the UI to fix. The button now finds the stale registration for this site's own endpoint, removes it, and registers again for a fresh secret. Registrations belonging to other sites on the same tenant are matched by exact URL and never touched.
