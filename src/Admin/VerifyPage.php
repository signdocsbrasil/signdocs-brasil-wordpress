...
final class VerifyPage
{
    ...
    private function renderEvidenceResult(VerificationResponse $r): void
    {
        ...
        $rows = [
            ['Evidence ID', (string) ($r->evidenceId ?? '')],
            ['Transaction ID', (string) ($r->transactionId ?? '')],
            ['Tenant CNPJ', (string) ($r->tenantCnpj ?? '')],
            ['Signer name', (string) ($r->signer['name'] ?? '')],
            ['Signer CPF/CNPJ', (string) ($r->signer['cpfCnpj'] ?? '')],
            ['Completed at', (string) ($r->completedAt ?? '')],
            ['Policy', (string) ($r->policy['profile'] ?? '')],
        ];
    ...
    private function renderEnvelopeResult(EnvelopeVerificationResponse $r): void
    {
        ...
        $signers = is_array($r->signers ?? null) ? $r->signers : [];
        if ($signers !== []) {
        ...
        $downloads = is_array($r->downloads ?? null) ? $r->downloads : [];
        if ($downloads !== []) {
