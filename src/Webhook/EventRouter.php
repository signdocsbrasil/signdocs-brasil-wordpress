...
final class EventRouter {
	...
	public function route( array $payload ): array {
		...
		switch ( $eventType ) {
			...
			case 'TRANSACTION.DEADLINE_APPROACHING':
				...
				Logger::warning(
					'nt65.deadline_approaching',
					'NT65 submission deadline approaching',
					array(
			...
			case 'QUOTA.WARNING':
				...
				Logger::warning(
					'api.quota_warning',
					'Tenant quota threshold crossed',
					array(
			...
			case 'API.DEPRECATION_NOTICE':
				Logger::warning(
					...
					'API deprecation webhook received',
					array(
		...
		Logger::info(
			'webhook.unmatched',
			'Webhook event did not match any CPT or handler',
			array(
				'eventType'     => $eventType,
				'sessionId'     => $sessionId,
				'transactionId' => $transactionId,
	...
	private function appendStepLog( int $postId, string $eventType, array $payload ): void {
		...
		$log = is_string( $raw ) && $raw !== '' ? json_decode( $raw, true ) : array();
		if ( ! is_array( $log ) ) {
			$log = array();
		}
