...
final class ResponseObserver
{
    ...
    public function __invoke(ResponseMetadata $metadata): void
    {
        try {
            ...
            Logger::warning('response.observer.error', $e->getMessage(), [
                'path' => $metadata->path,
    ...
    private function captureDeprecation(ResponseMetadata $meta): void
    {
        ...
        $notices = \get_option(self::DEPRECATION_NOTICE_OPTION, []);
        if (!is_array($notices)) {
            $notices = [];
        }
        ...
        Logger::warning('api.deprecation', "Deprecated endpoint {$meta->method} {$meta->path}", [
            'sunset' => $meta->sunset?->format('c'),
            'requestId' => $meta->requestId,
        ]);
    }
